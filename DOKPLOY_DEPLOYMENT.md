...
- **Cloudflare in front of Dokploy** (if used): Cloudflare's own edge request-body limit applies first (100MB on most plans by default, configurable lower per-hostname/per-rule via a Cloudflare Transform Rule or a Cloudflare-side "Response Buffering"/upload-limit setting on paid plans) — this is a useful additional layer but must not be treated as a substitute for the origin-level (Traefik/Dokploy) limit above, since Cloudflare's default is far larger than 5MB.
