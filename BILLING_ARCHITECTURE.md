...
## Mobile client integration (C2) — IMPLEMENTED, TESTED

`MOBILE_C2_REVENUECAT.md` has the full mobile-side detail; this section covers only the two new backend routes it calls, both in `backend/app/api/v2/billing.py`, mounted alongside `analyses`/`webhooks` in `app/main.py`. Neither is a new billing mechanism — both are thin, read-mostly adapters over the exact machinery documented above.

**`GET /api/v2/billing/status`** — read-only. User id comes exclusively from the authenticated bearer token (`rate_limit_by_user(GENERAL_POLICY)` → `get_current_user`); there is no request body or query parameter this route reads at all. Computes `analysis_allowance`/`analyses_remaining` via `build_entitlement_service()` (the same composition-root factory `analyses.py`/`analysis_worker.py` use — no independent allowance logic) and a new read-only helper, `usage_repository.get_current_period_usage_count()`, which reuses the exact `COUNT(*) ... WHERE status IN ('RESERVED','CONSUMED')` predicate `reserve()` itself uses internally, never a second, differently-worded count. Reads `user_entitlements` via `revenuecat_repository.get_entitlement()` on the ordinary `get_db_pool()` — safe because `skincare_app` still has a plain `SELECT` grant on that table (migration `9815eb266923` only revoked `INSERT`/`UPDATE`/`DELETE` from it). `has_premium_access` is true only for `ACTIVE`/`GRACE_PERIOD` — the identical predicate `RevenueCatEntitlementService.get_analysis_allowance()` uses, never redefined. When billing is disabled, reports the true free-tier state (`billing_enabled: false`, `environment: null`, `projection_status: null`) rather than fabricating a RevenueCat entitlement.

**`POST /api/v2/billing/sync`** — lets an authenticated client, immediately after a purchase or restore, request server reconciliation right now instead of waiting for webhook delivery. Requires `revenuecat_billing_enabled` (503 otherwise); rate-limited by a dedicated, bounded per-user policy (`BILLING_SYNC_POLICY`, `app/middleware/rate_limiter.py`, default 10/hour, `fail_open=False`) — never `GENERAL_POLICY`, since this route makes an outbound call to RevenueCat's own API on the caller's behalf and must not become an unlimited authenticated proxy to an external provider. Uses `get_billing_db_pool()` (the dedicated `skincare_billing` role) and reuses `RevenueCatReconciliationService.reconcile_user()` completely unmodified — no second RevenueCat REST implementation, no client-supplied `app_user_id` or entitlement claim of any kind accepted. On `ReconciliationAPIError`, `reconcile_user()` itself re-raises *before* writing anything to the local entitlement projection (see that class's own docstring), so this route's `503` response is a genuine "nothing changed, please retry" — never a downgrade of an existing entitlement merely because the network call to RevenueCat failed.

Tests: `backend/tests/api/test_billing_route.py` (19 tests) — user-id-from-auth-only, cross-user isolation, billing-disabled free tier, `ACTIVE`/`GRACE_PERIOD`/`EXPIRED`/`REVOKED` → premium/free access, sandbox-cannot-activate-production, allowance parity with `build_entitlement_service`, usage-remaining-never-negative, sync accepts no arbitrary user id, sync reconciles only the authenticated user, sync uses the billing pool, sync failure leaves the existing local entitlement byte-for-byte unchanged, the dedicated rate-limit policy, and that the ordinary `skincare_app` role still cannot write `user_entitlements` directly.
...
- Scheduling reconciliation automatically (cron/worker trigger) — `POST /api/v2/billing/sync` (Mobile C2) is an on-demand, client-triggered call, not the automatic scheduling this item asks for; that remains open.
...
- RevenueCat Billing/Stripe web checkout (C2 is native-purchasing-only; see `MOBILE_C2_REVENUECAT.md`'s "Web exclusion" section).
- Real RevenueCat Test Store / Apple / Google sandbox purchase smoke tests, and the external dashboard/store configuration they depend on (RevenueCat project, Test Store, `premium` entitlement, current Offering, paywall/Customer Center configuration, public SDK keys, iOS bundle identifier, Android package identifier, Apple/Google developer accounts) — see `MOBILE_C2_REVENUECAT.md`'s own "External configuration" and "Native build honesty" sections.
