...
**Commit this document describes:** the `feat/production-catalog-wave-1` branch, base `master` at `9aabba4ee8471f0dd048725f9c24725630c3a9c5` (the merged V1 account recovery / release foundation pass, PR #11), hardened by an independent review pass that closed three merge blockers (a publication-time backstop for `INSUFFICIENT_SOURCE_DATA` Wave 1 records, order-independent/trust-conflict-checked grouping with full merged-provenance preservation, and an idempotent-reimport reporting fix) — see "Independent review pass" below.
...
**Idempotent-reimport correctness (independent review).** A byte-identical reimport of the same manifest reuses the existing `catalog_import_batches` row (`content_sha256` uniqueness) rather than creating a new one. Independent review found the original implementation still wrote a NEW Wave 1 audit entry on every call regardless, so repeated idempotent reimports accumulated multiple audit entries for the same batch and `inspect-wave` summed all of them — a two-record manifest imported three times could report `total_records_supplied = 6` while the database still held exactly two import records. Fixed with defense in depth: (1) `import_manifest()` now writes the Wave 1 audit entry only when `ImportOutcome.batch_is_new` is `True` — a reused batch got its one true entry the first time it was genuinely imported, and nothing new happens on a reimport; (2) `_total_records_supplied_for_source()` additionally deduplicates by `batch_id` when reading (keeping only the earliest matching entry per batch), so even a duplicate entry from any other source could never inflate the total. Neither fix deletes or mutates audit history — this is a write-time gate plus a read-time dedup, never a retroactive edit. Proven: `tests/domain/test_catalog_wave_report.py::test_idempotent_reimport_does_not_inflate_total_records_supplied`, `::test_multiple_genuinely_distinct_batches_still_sum_correctly`.

## Tests — TESTED

`tests/domain/test_catalog_source_adapter.py` (47 tests, pure, no DB), `tests/domain/test_catalog_wave_service.py` (22 tests), `tests/domain/test_catalog_wave_report.py` (17 tests), `tests/catalog_admin/test_wave1_cli.py` (9 tests) — 95 Wave 1 tests total, all against real Postgres where DB access is involved (no mocked database anywhere in this pass, same convention as every prior catalog pass). Covers: valid manifest ingestion, malformed manifest record isolation, missing ingredient list, duplicate source record, duplicate product from the same source (including full order-independence), the same product from two sources, conflicting product identity, conflicting trust-critical formulation evidence, source-name/source-type binding, unresolved ingredient, source provenance persistence (including full multi-record evidence reconstruction), ingredient ordering preservation, verification state classification, publication only after validation, the direct publication-time `WAVE1_INSUFFICIENT_SOURCE_DATA` backstop (insufficient records cannot publish, sufficient ones still do), idempotent-reimport report-total correctness, formulation fingerprint stability/determinism, reformulation detection, old-formulation preservation, inactive-source rejection, restricted admin-role enforcement, the ordinary application role's inability to mutate catalog evidence through this pass's own new code path, and production data's inability to masquerade as a test fixture.
...
## Independent review pass — three merge blockers, all closed
