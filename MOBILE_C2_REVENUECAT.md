...
4. The backend route (`backend/app/api/v2/billing.py::sync_billing_status`) derives the user id from the bearer token only, requires `revenuecat_billing_enabled` and a configured `REVENUECAT_ENTITLEMENT_RESOURCE_ID` (fails closed with `503` otherwise), uses the dedicated `skincare_billing` DB pool, and calls `RevenueCatReconciliationService.reconcile_user()` — which calls RevenueCat's own real, documented `active_entitlements` REST resource and corrects the local `user_entitlements` projection on mismatch. **`active_entitlements[].entitlement_id` in that RevenueCat response is RevenueCat's internal entitlement resource ID (e.g. `entla1b2c3d4e5`), never the `premium` lookup key this mobile app itself uses** — the server matches it against a separate, backend-only `REVENUECAT_ENTITLEMENT_RESOURCE_ID` setting (never sent to or read by this mobile app; see `REVENUECAT_INTEGRATION_NOTES.md` section 5), while the local `user_entitlements` projection stays keyed by the `premium` lookup key exactly as before.
