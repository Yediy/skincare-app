...
- **`analysis_product_recommendations`**: one row per routine step that got a specific product match, `UNIQUE(analysis_request_id, plan_step_key)` — the same provenance `app/domain/recommendation_service.py`'s `StepProductRecommendation` carries in-memory, persisted for later audit/reconstruction. A step with no concrete match (Phase 11's fallback, or this pass's P0 fail-closed backstop — `PRODUCT_RECOMMENDATION_PIPELINE.md`) simply has no row here at all, never a placeholder. **Mobile V1 Phase C1** (migration `366861ec262d`) added nullable `brand_name_snapshot`/`product_name_snapshot`/`verification_date_snapshot` columns: a completed analysis is historical evidence, and these columns let it be rendered without re-resolving `product_id`/`formulation_id` against `products`/`brands`/`product_formulations`, which are mutable current-catalog state (a product can be renamed, a brand corrected, a formulation superseded after the analysis that recommended it). Populated for every new analysis from `StepProductRecommendation`'s own `brand`/`product_name`/`verification_date`; existing rows were left `NULL` rather than backfilled from current catalog data under a column name that claims historical provenance. A follow-up migration, `e421ed4cf053`, added a nullable `display_snapshot_version SMALLINT` marker column: whether a row's snapshot columns are authoritative is governed by this marker, never by per-field `NULL`-ness (a versioned row's `verification_date_snapshot` can be genuinely `NULL` — verification date is legitimately optional — and must not be papered over with a later catalog value). `commit_analysis_result()` stamps `display_snapshot_version = 1` on every row it writes; a `NULL` marker identifies a genuinely legacy, pre-`e421ed4cf053` row. See `PRODUCT_RECOMMENDATION_PIPELINE.md`'s "Client-facing projection" section for the read-time projection this enables.
...
Nullable; assigned idempotently by `UserPlacementService.get_placement()` from `settings.launch_home_region`/`launch_cell_id` the first time either is needed for a given user, never re-derived or overwritten once set.

**Correction:** an earlier draft of this document claimed "No RLS on `users` (unchanged — it never had any)." That was wrong even at the time it was written: migration `feb038fd05bd` (merged into this same branch, P0-1) enabled row-level security on `users` — see `SECURITY_AND_SAFETY_NOTES.md` and `ARCHITECTURE_CURRENT.md` for the full policy shape (`users_identity_scope`, scoped by `app.current_user_id`, same GUC as every other RLS-protected table in this document). `get_placement()`'s `UPDATE ... WHERE id = $1` is subject to that policy exactly like any other write to this table: it must run inside an explicit transaction that sets `app.current_user_id` to that same `$1` as its first statement, or `users_identity_scope`'s `WITH CHECK` silently turns the UPDATE into a zero-row no-op (RLS fails closed, not open) — the deployment's launch defaults would never actually persist. This was a real bug in an earlier version of `UserPlacementService`, fixed in the version now on this branch; the migration history itself (`feb038fd05bd`'s own comment) is left unedited, since it was already correct and has been applied — this note exists so this document stops contradicting it.
