...
**Display metadata provenance**: `brand`/`product_name`/`verification_date` prefer the row's own historical snapshot (`brand_name_snapshot`/`product_name_snapshot`/`verification_date_snapshot`, migration `366861ec262d` — see `ANALYSIS_DATA_MODEL.md`), governed by an explicit marker column, `display_snapshot_version` (migration `e421ed4cf053`), never by per-field `NULL`-ness. `commit_analysis_result()` stamps `display_snapshot_version = 1` on every row it writes, since it always captures the full display-snapshot contract at that moment even when `verification_date` is itself legitimately absent (a formulation can be recommended before it has ever been verified). `get_product_recommendations()` reads this as: `display_snapshot_version IS NOT NULL` → the row's snapshot columns are authoritative exactly as written, `NULL` or not, no current-catalog fallback on any individual field; `display_snapshot_version IS NULL` → a genuinely legacy row (older than `e421ed4cf053`), read-only join against the *current* catalog. A per-field `COALESCE(snapshot, current_catalog_value)` — this section's original design — could not tell a legacy row (fallback correct) apart from a versioned row whose `verification_date_snapshot` was genuinely `NULL` at analysis time, and would silently leak a *later* catalog verification timestamp into an already-completed analysis; closed as a merge-blocker in independent review before this phase's own initial pass had merged. This remains a read-time display convenience, never a write, and never a fabricated verification date or invented display name — an unresolvable field is `null`, and the client (`getProductDisplayLabel()`) renders "Product details unavailable" rather than inventing one. Proven by `tests/analysis/test_analysis_repository.py::test_get_product_recommendations_persists_and_prefers_display_snapshot` (snapshot wins even after the current catalog is mutated), `::test_get_product_recommendations_preserves_null_verification_date_on_versioned_row` (a versioned row's genuinely-`NULL` verification date stays `NULL` even after the catalog formulation is verified later), and `::test_get_product_recommendations_falls_back_to_current_catalog_when_legacy_marker_is_null` (a row constructed with a genuinely `NULL` marker, not merely omitted snapshot fields, falls back to the current catalog).
