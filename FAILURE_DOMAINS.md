...
## Mobile app connectivity during an in-flight analysis (`mobile/`)

**What happens if the device loses network mid-poll, or mid-submission?** `src/analysis/use-analysis-polling.ts` splits polling failures by `ApiError.retryable` (post-merge audit repair pass), not by treating every error identically. A TRANSIENT failure (network/timeout/5xx/retryable-429) never converts the analysis into `FAILED`: TanStack Query retains the last successfully fetched status across a failed background refetch, so the screen keeps showing the last known-good phase (`queued`/`processing`) plus a distinct "having trouble checking status" banner with its own Retry, while continuing to poll on its normal interval underneath. A PERMANENT failure (404/403/any other nonretryable 4xx — e.g. the analysis genuinely doesn't exist, or belongs to another session state entirely) stops automatic polling immediately and renders a dedicated "This analysis isn't available" state with a "Start over" action, rather than either polling forever or silently rendering as still-`queued` (the earlier implementation's actual defect, since fixed: it read a `status` field that stays `undefined` on a request that never once succeeded, and defaulted that straight to `"queued"` with no error check at all). A network failure *during submission itself* (before the backend ever durably receives the request) leaves both the captured photo and its `request_id` intact client-side — the capture screen's manual retry re-attempts with the exact same `request_id`, matching the backend's own idempotent-submit contract (`AnalysisSubmissionService`) exactly, so a retried submission after a genuinely-received-but-response-lost request never double-charges quota.

**What happens if the app is force-quit or the device restarts while an analysis is `QUEUED`/`PROCESSING`?** The analysis itself is entirely unaffected — it is real server-side state (`analysis_requests`), owned and driven to completion by the worker regardless of whether any mobile client is watching. What's honestly not implemented is *client-side resumption*: relaunching the app returns the user to the app home, not the polling screen, because no `analysis_id` is persisted anywhere to read back on restart (`OPEN_ENGINEERING_ITEMS.md` item 6a). The user is never shown a stale/wrong result — they simply don't see the in-progress one resume automatically, and would need to notice its result was never surfaced (a real, if minor, UX gap this document states plainly rather than glossing over). Separately: a force-kill or crash while a photo is captured but not yet submitted/discarded skips React's own unmount cleanup entirely (it cannot run in that scenario, and this codebase makes no claim that it can) — the local temp file is left for the OS cache directory's own eviction policy to eventually reclaim, not this app's own lifecycle cleanup. This is a local, single-device storage-hygiene gap, never a state-consistency or privacy-boundary one: the file never left the OS cache directory, was never uploaded, and is still deleted normally by every real exit path (success, retake, cancel, ordinary unmount) that actually runs.

**Can users still log in?** Yes — entirely unrelated.
**Can analysis continue?** Yes, server-side, regardless of the mobile client's own connectivity or lifecycle.
**Can data be lost?** No analysis state is lost. The one real, deliberate mobile-side gap is the restart-resumption UX above, not data loss.
**How is it restored?** Network: the existing poll loop and manual retry both recover automatically once connectivity returns. Restart-mid-analysis: not automatically restored this pass — tracked as item 6a.
**Blast radius:** One in-flight analysis attempt's visibility to its own initiating device/session — never other users, never the backend's own correctness.
