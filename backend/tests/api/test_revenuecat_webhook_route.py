...
concurrency, not about entitlement projection semantics (see
...
def _event_body(event_id: str, event_type: str = "INITIAL_PURCHASE") -> bytes:
    payload = {
        "api_version": "1.0",
        "event": {
            "id": event_id,
...
async def test_valid_webhook_accepted_creates_durable_event_and_job(client, app_db_pool):
    ...
    event_row = await app_db_pool.fetchrow(
...
async def test_duplicate_delivery_is_idempotent(client, app_db_pool):
    event_id = str(uuid.uuid4())
    body = _event_body(event_id)
    ...
    event_count = await app_db_pool.fetchval(
...
async def test_ten_concurrent_duplicate_deliveries_create_exactly_one_event_and_one_job(client, app_db_pool):
    event_id = str(uuid.uuid4())
    body = _event_body(event_id)
    headers = _headers(body)
