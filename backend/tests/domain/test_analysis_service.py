...
def real_singletons():
    ...
    return {"pipeline": pipeline, "scorer": scorer, "plan_service": plan_service}
...
async def test_perform_analysis_returns_a_result_with_no_http_involved(db_pool, app_db_pool, real_singletons):
    ...
    result = await perform_analysis(
        app_db_pool,
        AnalysisRequest(user_id=user_id, image_base64=image_base64),
        **real_singletons,
    )
...
async def test_perform_analysis_raises_invalid_image_for_bad_base64(db_pool, app_db_pool, real_singletons):
    ...
    with pytest.raises(InvalidImageError):
        await perform_analysis(
            app_db_pool,
            AnalysisRequest(user_id=user_id, image_base64="not-valid-base64!!!"),
            **real_singletons,
        )
