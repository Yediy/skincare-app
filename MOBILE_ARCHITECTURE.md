...
## Phase C2 — RevenueCat purchasing and entitlement sync — IMPLEMENTED, TESTED, INDEPENDENT-REVIEW REPAIRED (see DEFERRED for what stayed out)
...
**Adapter boundary** (`src/billing/revenuecat-adapter.ts`) -- a narrow `RevenueCatAdapter` interface (`isConfigured`/`configure`/`logIn`/`restorePurchases`/`presentPaywall`/`presentCustomerCenter`) is the only thing any screen or provider touches; no component imports `react-native-purchases`/`react-native-purchases-ui` directly. Deliberately has **no `logOut` method** -- see the identified-user lifecycle below. It also deliberately has no CustomerInfo-update-listener surface: an independent review found the original version registered one but nothing ever consumed the CustomerInfo it captured (see "CustomerInfo update listener removed" below). Every method signature was taken from the installed SDK's own TypeScript definitions (inspected directly, never invented from memory). Tests inject a plain, duck-typed fake adapter (`tests/billing/fake-adapter.ts`); no unit test requires a real RevenueCat account, native store, or network call.

**Identified-user lifecycle, fail-closed account switching** (`src/billing/revenuecat-context.tsx`, `RevenueCatProvider`, nested beneath `SessionProvider` in `app/_layout.tsx`) -- this app's canonical RevenueCat App User ID is the backend's own `users.id` UUID, obtained via `GET /me` only after `useSession().state.status === "AUTHENTICATED"`. RevenueCat is never configured while `SIGNED_OUT`/`UNKNOWN` (no unnecessary anonymous customer is ever created). A module-level singleton coordinator (`moduleConfiguredUserId`/`moduleConfiguringPromise`) guarantees `Purchases.configure()` runs at most once per native process even under concurrent/duplicate effect firing, and the installed SDK's own `Purchases.isConfigured()` facility is consulted as an additional guard (never a hand-invented equivalent). An independent review found the original version could re-expose a *previous* user's id to a *new* session when that new session's `logIn()` failed (`setConfiguredUserId(moduleConfiguredUserId)` ran unconditionally in a `finally`-adjacent path, even on failure) -- cross-account billing-state contamination. Fixed: `moduleConfiguredUserId` (module-level, "last user the SDK itself successfully configured/logged in as") and the React-exposed `configuredUserId` ("is the SDK demonstrably configured as *this session's* intended user right now") are now distinct. `configuredUserId` is set to the resolved UUID **only** immediately after that exact user's configure/switch succeeded, and explicitly set to `null` on any failure (`/me` failure, missing key, or a thrown `logIn()`) -- every purchase/restore/Customer-Center action gates on `configuredUserId`, so a stale previous-user id is never reachable, without ever calling `logOut()`. `useRevenueCat().retryIdentification()` re-attempts identification after such a failure, surfaced as a **Retry** action on the subscription screen.

**Sign-out / account switching** -- this app's custom-App-User-ID model means calling the SDK's `logOut()` would immediately mint an unwanted anonymous `$RCAnonymousID` customer, so ordinary application sign-out never calls it (there is nothing to call -- the adapter interface has no such method). Sign-out clears only the existing auth token pair, TanStack Query state (unchanged, pre-existing behavior), and C2's own in-memory billing UI state (`configuredUserId`); RevenueCat itself stays configured for whichever user it last identified. If a *different* account signs in during the same native process, the provider calls `Purchases.logIn()` with the new UUID -- never `logOut()` first. Proven directly (`tests/billing/revenuecat-context.test.tsx`'s account-switching test: user A configured, sign-out, user B signs in, RevenueCat transitions directly via `logIn`, zero `logOut`-equivalent calls at any point; and its fail-closed test: user B's `logIn()` throws, `configuredUserId` is `null` -- never user A's id -- until a successful `retryIdentification()`).

**Purchase-action readiness gate** (`src/billing/billing-action-readiness.ts::computeBillingActionReadiness`) -- an independent review found the subscription screen gated purchase actions on native `revenueCat.availability === "READY"` alone, ignoring the server's own `billing_enabled`/identity/entitlement-id agreement; a mobile SDK could be fully configured while the server explicitly reports billing disabled, letting a user attempt a store purchase the backend could never project (`POST /api/v2/billing/sync` would then 503). Fixed: this pure function returns `READY` only when native availability is `READY`, server billing status has loaded, `status.billing_enabled === true`, `revenueCat.configuredUserId === status.app_user_id`, and `revenueCat.entitlementId === status.entitlement_identifier` -- otherwise one of `MOBILE_UNAVAILABLE`/`SERVER_STATUS_UNAVAILABLE`/`BACKEND_DISABLED`/`IDENTITY_NOT_CONFIGURED`/`IDENTITY_MISMATCH`/`ENTITLEMENT_MISMATCH`, each rendered as an honest "Purchases unavailable" state. An existing server-granted entitlement is always still displayed regardless of this gate -- only the ability to start a *new* purchase action is gated.
...
**Paywall** (`src/billing/revenuecat-context.tsx::presentPaywall`, RevenueCatUI's `presentPaywallIfNeeded`) -- presents the RevenueCat dashboard's current/default Offering for the `premium` entitlement; no package/product identifier is hardcoded into purchase decision logic, so the dashboard can change presentation without a client release. `PAYWALL_RESULT.CANCELLED` is a true no-op. An independent review found `NOT_PRESENTED` was handled identically to `CANCELLED` -- but this app's Upgrade action is only reachable when the *server* has already said the user is not premium, so `NOT_PRESENTED` (RevenueCat's own contract: `presentPaywallIfNeeded` only presents when the entitlement isn't already locally active) is evidence the SDK's local cache and the server disagree. Fixed: `NOT_PRESENTED` now triggers the same `POST /api/v2/billing/sync` a real purchase would, rather than being silently dropped. A `PURCHASED`/`RESTORED` result never flips a local `hasPremium` flag directly -- it triggers `POST /api/v2/billing/sync`, then relies on the invalidated `useBillingStatusQuery` to refresh; premium UI activates only once the server confirms `ACTIVE`/`GRACE_PERIOD`.
...
**Customer Center** (`RevenueCatUI.presentCustomerCenter()`) -- exposed as "Manage subscription" for a premium/grace-period customer (subject to the purchase-action readiness gate above). After it closes, the same backend-sync-plus-refresh trigger fires, regardless of what happened inside it -- this app never attempts to cancel a subscription directly through the backend; the store/provider owns subscription management, and cancellation does not revoke access immediately if the server entitlement remains active through the paid period (the existing backend entitlement state machine is unmodified and remains authoritative). If `presentCustomerCenter()` itself throws, the screen shows a bounded error and does not sync -- nothing could have changed if it never successfully opened/closed.

**CustomerInfo update listener removed** -- the original version registered `addCustomerInfoUpdateListener`/`removeCustomerInfoUpdateListener` and stored the result in `customerInfo` state, with a comment claiming the subscription screen used it to trigger a sync. An independent review found that claim false: nothing anywhere read `customerInfo`, so the listener was a misleading pseudo-integration with no actual behavior. Removed entirely (adapter, context, and state) -- purchase/restore/Customer-Center flows already explicitly trigger reconciliation on their own; a passive listener added nothing. If a genuine need for push CustomerInfo updates arises later, it should be added back with an explicit, bounded, coalesced-sync consumer, not as a dangling side effect.
...
**Test coverage**: `mobile/tests/billing/` (identified-user lifecycle: waits for `/me`, configures once, account switching without `logOut`, fail-closed switching + retry, availability derivation for DISABLED/WEB_UNSUPPORTED/MISSING_KEY/READY, purchase-error classification, `computeBillingActionReadiness` for every named reason), `mobile/tests/query/use-billing.test.tsx` (server status is the source of truth, sync invalidates the status query), `mobile/tests/app/subscription.test.tsx` (every documented UI state, purchase-succeeded-but-sync-fails never unlocking premium locally, `NOT_PRESENTED` triggering sync while `CANCELLED` does not, the readiness gate for every mismatch reason, Customer Center error handling, and the web-unsupported path never calling a native method). Every test injects a fake adapter or mocks the query hooks directly -- zero real network/native purchase calls in a normal Jest run.
