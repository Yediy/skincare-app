...
## Catalog ingestion & administration — `VERIFIED_IMPLEMENTED` (this pass), CLI-only

Full detail in `CATALOG_INGESTION_ARCHITECTURE.md`. Summary: a controlled pipeline (`app/domain/catalog_ingestion_service.py`, `catalog_publication_service.py`, `catalog_review_service.py`) that safely feeds the *existing* normalized catalog (`PRODUCT_CATALOG_ARCHITECTURE.md`) without touching its schema or `SafetyEngine`. JSON/JSONL source files import into durable, immutable staging rows (`catalog_import_batches`/`catalog_import_records`, migration `2de8380d3618`), normalize against a typed, bounded Pydantic contract, resolve ingredients through the existing exact canonical/alias resolver only (never fuzzy — an unresolved name becomes an audited human-review item, `catalog_review_items`), and publish atomically (`CatalogPublicationService`, one transaction, `--dry-run` supported via rollback) into `brands`/`products`/`product_formulations`/`product_skus`/`formulation_ingredients`, with permanent provenance (`catalog_formulation_provenance`) and an append-only audit trail (`catalog_audit_log`). A new `product_formulations.publication_status` column (migration `37c88143a9ed`, six states, `DRAFT` default) is a genuinely distinct concept from `is_current`/`ingredient_data_status` — `ProductMatchingService`'s discovery query now requires `PUBLISHED` + `COMPLETE` + `is_current` simultaneously, closing the gap where any of those three alone would have been enough. A reformulation supersedes the old formulation and publishes the new one atomically, never leaving two `is_current` rows for the same product/market (database-enforced via the pre-existing partial unique index, proven under real concurrency). Database writes go through a dedicated, `NOLOGIN`-from-creation `skincare_catalog_admin` role (migration `13c1fff1867e`) — `skincare_app` retains its pre-existing `SELECT`-only posture on every catalog table, unchanged. No HTTP route; `python -m app.catalog_admin` (gated by `CATALOG_ADMIN_ENABLED`) is the only admin surface this pass ships. No web crawler or provider-specific acquisition adapter — this pass ingests bytes an operator already has in hand.
